...
import com.rbi.admin.dao.ower.*;
import com.rbi.admin.dao.structure.StructureDAO;
import com.rbi.admin.entity.edo.*;
...
public class OwnerInformationService{
    ...
    private final static Logger logger = LoggerFactory.getLogger(OwnerInformationService.class);
    ...
    @Autowired
    IRoomAndCustomerDAO roomAndCustomerDAO;

    @Autowired
    IRoomAndChargeItemsDAO roomAndChargeItemsDAO;
...
//        String filePath = "D:\\newExcel.xlsx";
        String columns[] = {"villageName","regionName","buildingName","unitName","floor","roomCode","roomSize",
...
//                  此处判断装修时间，为空不set值
                ...
                roomDO.setFloorCode(floorCode);
                if (2==roomStatus){
                    roomDO.setStartBillingTime(startBillingTime);
                    roomDO.setRealRecyclingHomeTime(realRecyclingHomeTime);
                }
...
//                roomAndCustomerDAO.deleteAllByRoomCode(roomCode);
                ...
                logOwnerInformationDO.setIdt(DateUtil.date(DateUtil.FORMAT_PATTERN));
                logOwnerInformationDO.setCode(i+2);
                logOwnerInformationDO.setResult("第"+(i+2)+"行，导入失败！其他错误");
                logOwnerInformationDOS.add(logOwnerInformationDO);
            }
        }
